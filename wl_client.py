...
def parse_request_file(filename) -> List[request]:
    with io.open(filename) as file:
        ...
        if not requests:
            print(f'No readable value in {filename}, using default request: '
                  f'"{PROTOCOL} {BENCH_TYPE} {METRICS} {BATCH_UNIT} {BATCH_ID} {BATCH_SIZE}"')
            requests.append(request(protocol=PROTOCOL,
...
def setup_arg_parser() -> argparse.ArgumentParser:
    ...
    single_parser.add_argument("bench_type", choices=["DVD-test", "DVD-training", "NDBench-test", "NDBench-training"],
